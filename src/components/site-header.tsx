  ...
  return (
    <header className="sticky top-0 z-50 border-b border-black/[0.06] bg-[#eef0f1]/92 backdrop-blur-xl dark:border-white/[0.07] dark:bg-[#12151a]/92">
      <div className="mx-auto flex h-[72px] w-full max-w-[1420px] items-center justify-between px-5 sm:px-7 lg:px-8">
        ...
        >
          {settings.brandName}
          <span className="text-[#f05a3d] transition-transform group-hover:translate-x-0.5">.</span>
        </Link>
          ...
          >
            <div>
              {links.map(([label, href], index) => (
                ...
                >
                  <span className="font-[var(--font-display)] text-[clamp(34px,10vw,54px)] font-semibold tracking-[-0.055em]">{label}</span>
                  <span className="font-[var(--font-mono)] text-[9px] text-black/30 dark:text-white/30">0{index + 1}</span>
                </Link>
              ))}
              <Link
                href="/resume"
                ...
                className="group flex items-center justify-between border-b border-black/8 py-5 dark:border-white/8"
              >
                <span className="font-[var(--font-display)] text-[clamp(34px,10vw,54px)] font-semibold tracking-[-0.055em]">Resume</span>
                <span className="font-[var(--font-mono)] text-[9px] text-black/30 dark:text-white/30">05</span>
              </Link>
            </div>

            <div className="flex items-center justify-between pt-8">
              <span className="max-w-[220px] font-[var(--font-mono)] text-[9px] uppercase leading-5 tracking-[0.1em] text-black/35 dark:text-white/35">{settings.role}</span>
